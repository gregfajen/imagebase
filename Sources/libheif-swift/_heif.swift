...
func heif_read(data: Data) throws -> HEIFImage {
    ...
    var error: heif_error?
    
    if heif_image_handle_has_alpha_channel(handle) == 0 {
        error = heif_decode_image(handle, &image, heif_colorspace_RGB, heif_chroma_interleaved_RGB, nil)
    } else {
        error = heif_decode_image(handle, &image, heif_colorspace_RGB, heif_chroma_interleaved_RGBA, nil)
    }
    
    if let error = error, error.code != heif_error_Ok {
        throw error
    }
